...
CASMSOURCES?=
CASMOUTPUT=$(CASMSOURCES:.casm=.h) $(CASMSOURCES:.cpasm=.h)

# Assume copasm files will generate header files
HEADERS+=$(wildcard *.h) $(addsuffix .h,$(basename $(wildcard *.casm))) $(addsuffix .h,$(basename $(wildcard *.cpasm)))
...
%.h : %.cpasm
	@mkdir -p $(@D)
	$(CPP) -D__COPASM__=1 -I$(XOSERA_M68K_API) $< -o $(basename $<).casm.ii
...
clean:
	$(RM) $(OBJECTS) $(CASMOUTPUT) $(ELF) $(BINARY) $(MAP) $(SYM) $(SYM_SIZE) $(DISASM)
	$(RM) $(addsuffix .lst,$(basename $(wildcard *.casm) $(wildcard *.cpasm) $(wildcard *.S) $(wildcard *.asm)))
	$(RM) $(addsuffix .casm.ii,$(basename $(wildcard *.cpasm))) $(addsuffix .h,$(basename $(wildcard *.casm) $(wildcard *.cpasm)))
...
.PHONY: all clean dump disasm load test term mactest macterm
