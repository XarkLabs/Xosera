...
static void sllog(uint16_t channel, uint16_t addr, uint16_t chunk_start, uint16_t chunk_end, uint16_t loaded_size, pointer_t sample_addr) {
...
static inline uint16_t load_sample_chunk(
        uint8_t xosera_channel,
...
static inline uint16_t load_next_chunk(Channel *channel) {
    if (channel->next_chunk_start >= channel->current_sample->length) {
        if (channel->current_sample->sample->repeat_length > 1) {
            ltlog(channel->xosera_channel, channel->current_sample->sample->repeat_point, channel->current_sample->sample->repeat_length);
    ...
    uint16_t result = load_sample_chunk(
...
static void start_channel_sample(
        Channel *channel, 
        PtMemorySample *sample,
        uint16_t period
) {
...
static inline void xosera_set_period_vol_start_length(uint8_t xosera_channel, uint16_t period, uint16_t vol, uint16_t start,  uint16_t len) {
    switch (xosera_channel) {
    case 0: 
        xreg_setw(AUD0_PERIOD, period);
    ...
    if (channel->current_sample == &silence) {
        ...
        if (actualLoaded == 0) {
            ...
            xosera_set_period_start_length(
                channel->xosera_channel, 
                channel->period, 
                channel->next_buffer_start == channel->buffer_b_addr 
                        ? channel->buffer_a_addr 
                        : channel->buffer_b_addr,
...
static inline void handleEffect(uint16_t effect, Channel *channel) {
    if ((effect & 0xF00) == 0xB00) {
        ...
        vslog(channel->xosera_channel, channel->current_volume, (effect & 0xFF), current_effect->paramx, current_effect->paramy);
        ...
        for (int mod_channel = 0; mod_channel < 4; mod_channel++) {            
            ...
            Channel *channel = mod_channel == 0 ? &channel0 : 
                    mod_channel == 1 ? &channel1 :
                        mod_channel == 2 ? &channel2 : &channel3;
            ...
            // Handle these *after* loading so they can take effect immediately if a sample is also played on this division...
