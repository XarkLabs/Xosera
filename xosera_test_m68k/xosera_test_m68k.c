...
void test_blit()
{
    ...
    do
    {
        ...
        wait_blit_ready();
        xreg_setw(BLIT_CTRL, 0xEE03);             // constA, constB
        ...
        xreg_setw(BLIT_SRC_B, 0x0101);            // B AND const (non-zero to disable transparency)
        ...
        for (int i = 0; i < 1024; i++)
        {
            for (int b = 0; b < nb; b++)
            {
                ...
                wait_blit_ready();                   // make sure blit ready (previous blit started)
                xreg_setw(BLIT_CTRL, 0xEE03);        // constA
                xreg_setw(BLIT_SHIFT,
                ...
                xreg_setw(BLIT_SRC_B, 0x0101);                        // B source+transp VRAM addr (moto_m)
                xreg_setw(BLIT_VAL_C, 0x0000);                        // C XOR const
